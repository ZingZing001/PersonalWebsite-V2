      ...
      <div className="container max-w-6xl mx-auto">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-12">
          {categories.map((category,key) => (
            <button
              ...
              className={cn("px-5 py-2 rounded-full transition-all duration-300 capitalize relative group border",
                activeCategory === category
                ? "bg-primary text-primary-foreground hover:bg-primary/90 shadow-lg border-primary/50"
                : "bg-card/90 text-foreground hover:bg-primary/50 dark:border-border/50 border-primary/30"
              )}>
                {category}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {filteredSkills.map((skill,key) => {
            ...
            <div 
              key={key} 
              className="bg-card/90 border-2 dark:border-border/50 border-primary/20 rounded-lg p-6 shadow-lg dark:shadow-primary/5 shadow-xl card-hover animate-fade-in"
